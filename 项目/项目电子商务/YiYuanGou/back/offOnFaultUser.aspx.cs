...
public partial class back_offOnFaultUser : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            if (Session["UMode"] != null && !Session["UMode"].ToString().Equals("0"))
            {
                ...
                Response.Write("<script>alert('您没有权限开启虚拟用户功能'); window.location.href='../back_index.aspx';</script>");
            }
            else
            {
                if (!IsPostBack)
    ...
    {
        string sqlstr = "select roomID,product.productID,proName,proSize,price,color,roomType from Product,room,product_cata where product.productID=room.productID and product.pcid=product_cata.pcid and room.statement='0' ";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (sql != sql1)
        {
            ...
            sql = "update room set statement=1 " + sql + ")";
