...
public partial class back_setFaultUser : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            if (Session["UMode"] != null && !Session["UMode"].ToString().Equals("0"))
            {
                ...
                Response.Write("<script>alert('您没有权限设置虚拟用户比例'); window.location.href='../back_index.aspx';</script>");
            }
            else
            {
    ...
    {
        string sqlstr = "select roomID,product.productID,proName,proSize,price,color,roomType,newFaultUser,falseuser/limiteduser*100 as currRatio from Product,room,product_cata where product.productID=room.productID and product.pcid=product_cata.pcid and room.statement='1' ";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        {
            string rid = this.GridView1.Rows[i].Cells[0].Text;
            TextBox tb = this.GridView1.Rows[i].FindControl("TextBox4") as TextBox;
            sql = "update room set newFaultUser = " + tb.Text + " where roomID="+rid;
            DBConn.ExecuteUpdate(sql);           
        }
        Response.Write("<script>alert('虚拟用户比例设置成功！');</script>");
        sqlSource();
    }
