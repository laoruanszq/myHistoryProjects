...
public partial class back_setHotRoom : System.Web.UI.Page
{
    ...
    {
        string sqlstr = "select roomID,product.productID,proName,proSize,price,color,roomType,isopen from Product,room,product_cata where product.productID=room.productID and product.pcid=product_cata.pcid ";
        ...
        {
            sqlstr = sqlstr + " and isopen='1'";
            s = 1;
        }
        else
        {
            sqlstr = sqlstr + " and isopen='0'";
            s = 0;
        }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (sql != sql1)
        {
            ...
            if(s==0)
                sql = "update room set isopen ='1'  " + sql + ")";
            else
                sql = "update room set isopen ='0'  " + sql + ")";
            DBConn.ExecuteUpdate(sql);
            if(s==0)
                Response.Write("<script>alert('房间开启成功！');</script>");
            else
                Response.Write("<script>alert('房间关闭成功！');</script>");
            sqlSource();
        }        
    }
