...
public partial class front_ProductListFree : System.Web.UI.Page
{
    ...
    {
        string sqlstr_brand = "select distinct  brand from Product where IsJingyanbi=1 and PCID like '" + sort +"%'";
    ...
    {
        //得到sql语句
        string sqlstr = "select * from Product ";     //sql前部分语句
        string sqlstr2 = "where IsJingyanbi=1 ";          //sql where语句
    ...
    public void bindingtree()
    {
        ...
        if (ds.Tables[0] != null && ds.Tables[0].Rows.Count != 0)
        {
            for (int c1 = 0; c1 < ds.Tables[0].Rows.Count; c1++)
            {
                ...
                tnClass1.NavigateUrl = "ProductListFree.aspx?sort=" + ds.Tables[0].Rows[c1][1].ToString() + "&keyword=non&isbool=non&brand=non";
                ...
                if (ds.Tables[1] != null && ds.Tables[1].Rows.Count != 0)
                {
                    for (int c2 = 0; c2 < ds.Tables[1].Rows.Count; c2++)
                    {
                        if (tnClass1.Text == ds.Tables[1].Rows[c2][1].ToString())
                        {
                            ...
                            tnClass2.NavigateUrl = "ProductListFree.aspx?sort=" + ds.Tables[1].Rows[c2][2].ToString() + "&keyword=non&isbool=non&brand=non";
                            if (ds.Tables[2] != null && ds.Tables[2].Rows.Count != 0)
                            {
                                for (int c3 = 0; c3 < ds.Tables[2].Rows.Count; c3++)
                                {
                                    if (tnClass2.Text == ds.Tables[2].Rows[c3][2].ToString())
                                    {
                                        ...
                                        tnClass3.NavigateUrl = "ProductListFree.aspx?sort=" + ds.Tables[2].Rows[c3][0].ToString() + "&keyword=non&isbool=non&brand=non";
    ...
    {
        //表单验证，只能输入数字
        Response.Redirect("ProductListFree.aspx?price=is&min=" + TextBox1.Text + "&max=" + TextBox2.Text);
    }
    protected void ResetPrice_Click(object sender, EventArgs e)
    {
        Response.Redirect("ProductListFree.aspx?price=non");
    }
